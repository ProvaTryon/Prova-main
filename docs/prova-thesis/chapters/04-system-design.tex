...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/arch-overview.png}
...
\subsection{Prova-VTON Model Design and Dataset}
The core of the try-on capabilities is powered by Prova-VTON, a custom diffusion-based generative model. Unlike systems relying on generic pre-trained zero-shot models, Prova-VTON was explicitly trained to handle the complex geometric transformations required for both 2D and multi-angle scenarios. The model architecture is designed with view-consistent conditioning and cross-view attention mechanisms to preserve garment identity across different camera angles. To achieve high fidelity, the model was trained on the MVHumanNet dataset, a large-scale multiview human dataset containing thousands of diverse human identities, poses, and clothing combinations captured from multiple camera viewpoints. This training enables Prova-VTON to natively understand 3D body priors and synthesize view-consistent garment deformations without requiring explicit geometric warping.
